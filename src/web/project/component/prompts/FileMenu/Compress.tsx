...
export function Compress(props) {
    ...
    const confirm = ()=> {
        ...
        req.filePaths = files;
        req.format = format;
        req.tar_filename = `${getRouterAfter('file',location.pathname)}${tar_filename}`;
            ...
            <CardPrompt title={t("压缩文件")} cancel={close} confirm={confirm} cancel_t={t("取消")} confirm_t={t("确定")}
                        context={!progress ? [
                                <Select value={format} onChange={(value)=>{
                                    ...
                                    setFormat(value);
                                    setPlaceholder(`.${v}`)
                                }} options={[{title:`tar ${t("格式")}`,value:FileCompressType.tar},{title:`zip ${t("格式")}`,value:FileCompressType.zip},{title:`gz ${t("格式")}`,value:FileCompressType.gzip}]}/>,
                                <Select value={compress_level} onChange={(value)=>{
                                    const v = parseInt(value);
                                    setCompress_level(v);
                                }} options={levels}/>,
                                <InputText placeholder={placeholder} placeholderOut={t("压缩到")} value={tar_filename} handleInputChange={(value) => setTar_filename(value)}/>] :
                            [<div>{t("进度")}:</div>,
