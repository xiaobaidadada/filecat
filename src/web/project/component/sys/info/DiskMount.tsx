...
const Lvm = (props: { name: string, lvms: { name: string }[] }) => {
    ...
    return (
        <div
            style={{border: '2px solid black', padding: '10px', width: '300px', height: '300px', position: 'relative'}}>
            {props.name}
                ...
                <Row>
                    <Column widthPer={40}>
                        <Card title={t("物理块(硬盘)设备、逻辑卷、物理卷")} titleCom={<div>
                            {check_user_auth(UserAuth.sys_disk_mount) &&
                            ...
                            <TreeView list={list} render={node_render_pd} onContextMenu={handleContextMenu}
                                      click={(item) => {
                                          ...
                                          set_info_list([[<TextTip>{mountpoint}</TextTip>,
                                              <TextTip>{item.uuid}</TextTip>, t(item.type), t(item.fstype), item.fssize, item.size,
                                              <TextTip>{item.path}</TextTip>, item.pttype]]);
                                      }}/>
                        </Card>
                    </Column>
                    <Column widthPer={60}>
